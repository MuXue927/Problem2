...
input_loc = os.path.join(par_path, 'datasets', 'multiple-periods', 'large')
output_loc = os.path.join(par_path, 'OutPut-CG', 'multiple-periods', 'large')
...
def run_model(input_file_loc=input_loc, output_file_loc=output_loc):
    ...
    # =========================================================
    # 初始化主模型, 包含所有周期的约束条件, 只需要初始化一次
    masterM = MasterModel(data)
    
    # 初始化子模型, 子模型需要固定生产基地 i, 经销商 j, 车辆类型 f, 周期 t
    subM = {}
    for day_id in range(1, data.horizons + 1):
        triple_plant_dealer_veh = find_triple_plant_dealer_veh(masterM)
    ...
    # 执行优化过程
    log_printer.print(f"Start Solving! / Now {datetime.datetime.now()}")
    # 使用列生成算法，获取主模型松弛问题的可行解
    return_value = opt_master_model(model=masterM)
    if not return_value:
        return False
    # 更改主模型的决策变量类型，重新优化，拿到主模型的整数解
    return_value = construct_int_solution(model=masterM, day=day_id)
    if not return_value:
        return False
    
    update_demands(rmp=masterM)
    update_inventory(rmp=masterM)
    # 保存计算结果
    output_result(model=masterM)
...
def find_triple_plant_dealer_veh(model: MasterModel):
    global data
    target_dealers = {dealer_id for (dealer_id, sku_id) in model.cons_dealer_total_demands}
    duet_dealer_veh = [
...
def opt_master_model(model: MasterModel):
    global data, subM
    ...
    while True:
        ...
        PIs_1 = {key: model.cons_dealer_total_demands[key].Pi for key in model.cons_dealer_total_demands}
        PIs_2 = {key: model.cons_dealer_demand[key].Pi for key in model.cons_dealer_demand}
        PIs_3 = {key: model.cons_plant_supply[key].Pi for key in model.cons_plant_supply}
        PIs_4 = {key: model.cons_sku_transfer[key].Pi for key in model.cons_sku_transfer}
        
        
        opt_s_ikt = {key: model.s_ikt[key].X for key in model.s_ikt}
        ...
        quartet = [
            (plant, dealer, veh_type, day) 
            for plant, dealer, veh_type in triple_plant_dealer_veh 
            for day in range(1, data.horizons + 1)
        ]
        random.shuffle(quartet)

        for i, (plant, dealer, veh_type, day) in enumerate(quartet):  # 求解每个子模型，构造装载模式
            sub_model = subM[plant, dealer, veh_type, day]
            pis1 = gp.tupledict({sku: PIs_1[dealer, sku] for sku in sub_model.require_skus})
            pis2 = gp.tupledict({sku: PIs_2[dealer, sku, day] for sku in sub_model.require_skus})
            pis3 = gp.tupledict({sku: PIs_3[dealer, sku, day] for sku in sub_model.require_skus})
            pis4 = gp.tupledict({sku: PIs_4[plant, sku, day] for sku in sub_model.require_skus})
            
            # 子模型的目标函数
            sub_model.obj = sub_model.d_xs.prod(pis1) + sub_model.d_xs.prod(pis2) + \
                sub_model.d_xs.prod(pis3) + sub_model.d_xs.prod(pis4) - \
                data.veh_type_cost[veh_type] - sub_model.p_f
                
            sub_model.model.update()
            # sub_model.model.write(f"SubModel_Day_{day}_{i+1}.lp")
        ...
        if model.num_patterns > data.param_trigger_shrink_level:
            model.run()
            if check_artificial_variables(model):  # 不存在人工变量, 则返回True
                model.shrink()
                return True
            else:
                return False
...
def check_artificial_variables(model: MasterModel):
    """检查主模型中是否存在人工变量, 如果存在, 则返回False, 否则返回True"""
    if model.get_alpha_size() != 0 or model.get_beta_size() != 0 or model.get_gamma_size() != 0 \
        or model.get_epsilon_size() != 0 or model.get_fai_size() != 0:
        log_printer.print("Exception: Artificial variables exists in master model!", color='bold red')
        # 查看人工变量的取值
        for (dealer, sku_id), var in model.d_alpha.items():
            if var.X > 0:
                log_printer.print(f"d_alpha{[dealer, sku_id]} = {var.X}")
        for (dealer, sku_id, day), var in model.d_beta.items():
            if var.X > 0:
                log_printer.print(f"d_beta{[dealer, sku_id, day]} = {var.X}")
        for (dealer, sku_id, day), var in model.d_gamma.items():
            if var.X > 0:
                log_printer.print(f"d_gamma{[dealer, sku_id, day]} = {var.X}")
        for (plant, sku_id, day), var in model.d_epsilon.items():
            if var.X > 0:
                log_printer.print(f"d_epsilon{[plant, sku_id, day]} = {var.X}")
        for (plant, sku_id, day), var in model.d_fai.items():
            if var.X > 0:
                log_printer.print(f"d_fai{[plant, sku_id, day]} = {var.X}")
        return False
    return True
...
def construct_int_solution(model: MasterModel, day: int):
    ...
    # 到这里表示, 模型中不存在人工变量, 且求得最优解, 输出使用的车辆数量
    num_veh_used = sum(round(var.X) for key, var in model.d_xs.items() if var.X > 0)
    log_printer.print(f"The number of vehicles used is {num_veh_used}")
...
def complete_pattern(model: MasterModel, day: int):
    ...
    for (plant, dealer) in data.construct_supply_chain():
        ...
        complement_sku = {}
        for sku in sku_set:
            cons1 = model.cons_dealer_total_demands[dealer, sku]
            LHS1 = model.model.getRow(cons1)
            
            cons2 = model.cons_dealer_demand[dealer, sku, day]
            LHS2 = model.model.getRow(cons2)
            
            if LHS1.size() > 1 and LHS2.size() > 1:
                continue

            complement_sku[sku] = data.demands[dealer, sku]
...
def clear_output_file(output_file_loc=output_loc):
    ...
    files_to_clear = {
        ...
        'extral_fulfill.csv': ['client_code', 'product_code', 'volume']
    }
    ...
    for (plant, dealer, veh_type, day, pattern_id), var in rmp.d_xs.items():
        if var.X > 0:
            ...
            for (sku_id, day), qty in load_pattern.items():
                if (dealer, sku_id) in data.demands:  # 同理, 这里也需要检查demands中是否存在当前经销商和SKU的组合
                    data.demands[dealer, sku_id] -= round(var.X * qty)
...
def create_dataframe(rmp: MasterModel):
    ...
    for (plant, dealer, veh_type, day, pattern_id), var in rmp.d_xs.items():
        if var.X > 0:
            pattern_name = var.VarName
            # 当前装载模式对应的车辆编号集合
            veh_ids = pattern_veh_id_map[pattern_name]
            while len(veh_ids) != 0:
                veh_id = veh_ids.pop()
                load_pattern = rmp.pattern_content[pattern_name]
                for (sku_id, day), qty in load_pattern.items():
...
def result_state(r_data: InputDataCG):
    ...
    if is_cap_exceed.all():  # 所有值都为True，则返回True
        ...
        log_printer.print("Warning! The volume of SKUs loaded in some vehicles has exceeded their capacities.", color='bold yellow')
