...
from optutility import my_callback
...
@dataclass
class MasterModel:
    # cur_day: int     # 当前的规划周期, 在主模型中, 不能将每个周期分开考虑
    data: InputDataCG
    # 对于多周期问题，默认规划周期长度为 1
    ...
    # 人工变量 alpha 用于约束 cons_dealer_total_demands
    d_alpha: gp.tupledict[Tuple[str, str], gp.Var] = field(init=False)  # {(dealer, sku): gp.Var}
    
    # 人工变量 beta 用于约束 cons_dealer_demand
    d_beta: gp.tupledict[Tuple[str, str, int], gp.Var] = field(init=False)  # {(dealer, sku, day): gp.Var}
    # 人工变量 gamma 用于约束 cons_plant_supply
    d_gamma: gp.tupledict[Tuple[str, str, int], gp.Var] = field(init=False)  # {(dealer, sku, day): gp.Var}
    
    # 人工变量 epsilon 和 人工变量 fai 用于约束 cons_sku_transfer
    d_epsilon: gp.tupledict[Tuple[str, str, int], gp.Var] = field(init=False)  # {(plant, sku, day): gp.Var}
    d_fai: gp.tupledict[Tuple[str, str, int], gp.Var] = field(init=False)  # {(plant, sku, day): gp.Var}
    ...
    # 定义需要向模型中添加的约束条件
    # 在所有周期内，所有生产基地运往某个经销商 j 的SKU k 的总量，大于等于经销商 j 对SKU k 的需求
    cons_dealer_total_demands: gp.tupledict[Tuple[str, str], gp.Constr] = field(init=False)  # {(dealer, sku_id): gp.Constr}
    
    # 在周期 t 内，从所有生产基地运往经销商 j 的SKU k 的数量，不超过
    # 该经销商在所有周期内对这种SKU的总需求数量
    cons_dealer_demand: gp.tupledict[Tuple[str, str, int], gp.Constr] = field(init=False)  # {(dealer, sku_id, day): gp.Constr}
    
    # 在周期 t 内，从所有生产基地运往经销商 j 的SKU k 的数量，不超过
    # 所有生产基地在周期 t 内可以提供的这种SKU的数量
    cons_plant_supply: gp.tupledict[Tuple[str, str, int], gp.Constr] = field(init=False)  # {(dealer, sku_id, day): gp.Constr}
    ...
    def __post_init__(self):
        ...
        duet = {(dealer, sku_id) for (dealer, sku_id) in self.demands}
        self.d_alpha = self.model.addVars(duet, lb=0, vtype=GRB.CONTINUOUS, name="d_alpha")
        
        
        triple = {(dealer, sku_id, day) for (dealer, sku_id) in self.demands 
                  for day in range(1, self.horizons + 1)}
        self.d_beta = self.model.addVars(triple, lb=0, vtype=GRB.CONTINUOUS, name="d_beta")
        self.d_gamma = self.model.addVars(triple, lb=0, vtype=GRB.CONTINUOUS, name="d_gamma")
        
        triple_plant_dealer_sku = {
            (plant, dealer, sku_id) for (plant, dealer), skus in self.data.construct_supply_chain().items()
            for sku_id in skus
        }
        self.triple_plant_sku_day = {(plant, sku_id, day) for (plant, dealer, sku_id) in triple_plant_dealer_sku
                                     for day in range(1, self.horizons + 1)}
        self.d_epsilon = self.model.addVars(self.triple_plant_sku_day, lb=0, vtype=GRB.CONTINUOUS, name="d_epsilon")
        self.d_fai = self.model.addVars(self.triple_plant_sku_day, lb=0, vtype=GRB.CONTINUOUS, name="d_fai")
        ...
        self.obj = (self.data.param_pun_factor1 * self.d_alpha.sum('*', '*') +
                    ...
                    self.data.param_pun_factor4 * self.d_epsilon.sum('*', '*', '*') +
                    self.data.param_pun_factor5 * self.d_fai.sum('*', '*', '*')
                    )
        
        self.model.setObjective(self.obj, sense=GRB.MINIMIZE)

        self.cons_dealer_total_demands = self.model.addConstrs((
            self.d_alpha[dealer, sku_id] >= self.demands[dealer, sku_id]
            for (dealer, sku_id) in duet
        ), name="cons_dealer_total_demands")
        
        self.cons_dealer_demand = self.model.addConstrs((
            self.d_beta[dealer, sku_id, t] >= -1 * self.demands[dealer, sku_id]
            for (dealer, sku_id, t) in triple
        ), name="cons_dealer_demand")
        
        self.cons_plant_supply = self.model.addConstrs((
            self.d_gamma[dealer, sku_id, t] >= -1 * (
                sum(self.sku_prod_each_day.get((plant, sku_id, t), 0) 
                    for plant in self.plants if sku_id in self.skus_plant[plant]) + \
                        self.s_ikt.sum('*', sku_id, t-1)
            )
            for (dealer, sku_id, t) in triple
        ), name="cons_plant_supply")
        
        self.cons_sku_transfer = self.model.addConstrs((
            self.d_epsilon[plant, sku_id, t] - self.d_fai[plant, sku_id, t] ==
    ...
    def get_alpha_size(self):
        non_zero_alpha = {(dealer, sku_id) for (dealer, sku_id), var in self.d_alpha.items() if var.X > POSITIVEEPS}
        return len(non_zero_alpha)

    def get_beta_size(self):
        non_zero_beta = {(dealer, sku_id, t) for (dealer, sku_id, t), var in self.d_beta.items() if var.X > POSITIVEEPS}
        return len(non_zero_beta)

    def get_gamma_size(self):
        non_zero_gamma = {(dealer, sku_id, t) for (dealer, sku_id, t), var in self.d_gamma.items() if var.X > POSITIVEEPS}
        return len(non_zero_gamma)
    
    def get_epsilon_size(self):
        non_zero_epsilon = {(plant, sku_id, t) for (plant, sku_id, t), var in self.d_epsilon.items() if var.X > POSITIVEEPS}
        return len(non_zero_epsilon)

    def get_fai_size(self):
        non_zero_fai = {(plant, sku_id, t) for (plant, sku_id, t), var in self.d_fai.items() if var.X > POSITIVEEPS}
        return len(non_zero_fai)

    
    def deactivate_alpha(self):
        for (dealer, sku_id) in self.d_alpha:
            self.d_alpha[dealer, sku_id].setAttr('UB', 0)

    def deactivate_beta(self):
        for (dealer, sku_id, t) in self.d_beta:
            self.d_beta[dealer, sku_id, t].setAttr('UB', 0)

    def deactivate_gamma(self):
        for (dealer, sku_id, t) in self.d_gamma:
            self.d_gamma[dealer, sku_id, t].setAttr('UB', 0)
            
    def deactivate_epsilon(self):
        for (plant, sku_id, t) in self.d_epsilon:
            self.d_epsilon[plant, sku_id, t].setAttr('UB', 0)

    def deactivate_fai(self):
        for (plant, sku_id, t) in self.d_fai:
            self.d_fai[plant, sku_id, t].setAttr('UB', 0)

    def deactivate_artificial_vars(self):
        self.deactivate_alpha()
        self.deactivate_beta()
        self.deactivate_gamma()
        self.deactivate_epsilon()
        self.deactivate_fai()

    def activate_alpha(self):
        for (dealer, sku_id) in self.d_alpha:
            self.d_alpha[dealer, sku_id].setAttr('UB', GRB.INFINITY)

    def activate_beta(self):
        for (dealer, sku_id, t) in self.d_beta:
            self.d_beta[dealer, sku_id, t].setAttr('UB', GRB.INFINITY)

    def activate_gamma(self):
        for (dealer, sku_id, t) in self.d_gamma:
            self.d_gamma[dealer, sku_id, t].setAttr('UB', GRB.INFINITY)
    
    def activate_epsilon(self):
        for (plant, sku_id, t) in self.d_epsilon:
            self.d_epsilon[plant, sku_id, t].setAttr('UB', GRB.INFINITY)

    def activate_fai(self):
        for (plant, sku_id, t) in self.d_fai:
            self.d_fai[plant, sku_id, t].setAttr('UB', GRB.INFINITY)

    def activate_artificial_vars(self):
        self.activate_alpha()
        self.activate_beta()
        self.activate_gamma()
        self.activate_epsilon()
        self.activate_fai()
    ...
    def remove_partial_artificial_vars(self):
        ...
        """
        remove_alpha = {(dealer, sku_id): var for (dealer, sku_id), var in self.d_alpha.items() if var.X <= POSITIVEEPS}
        reserve_alpha = {(dealer, sku_id): var for (dealer, sku_id), var in self.d_alpha.items() if var.X > POSITIVEEPS}
        self.d_alpha = gp.tupledict(reserve_alpha)

        remove_beta = {(dealer, sku_id, t): var for (dealer, sku_id, t), var in self.d_beta.items() if var.X <= POSITIVEEPS}
        reserve_beta = {(dealer, sku_id, t): var for (dealer, sku_id, t), var in self.d_beta.items() if var.X > POSITIVEEPS}
        self.d_beta = gp.tupledict(reserve_beta)

        remove_gamma = {(dealer, sku_id, t): var for (dealer, sku_id, t), var in self.d_gamma.items() if var.X <= POSITIVEEPS}
        reserve_gamma = {(dealer, sku_id, t): var for (dealer, sku_id, t), var in self.d_gamma.items() if var.X > POSITIVEEPS}
        self.d_gamma = gp.tupledict(reserve_gamma)
        
        remove_epsilon = {(plant, sku_id, t): var for (plant, sku_id, t), var in self.d_epsilon.items() if var.X <= POSITIVEEPS}
        reserve_epsilon = {(plant, sku_id, t): var for (plant, sku_id, t), var in self.d_epsilon.items() if var.X > POSITIVEEPS}
        self.d_epsilon = gp.tupledict(reserve_epsilon)

        remove_fai = {(plant, sku_id, t): var for (plant, sku_id, t), var in self.d_fai.items() if var.X <= POSITIVEEPS}
        reserve_fai = {(plant, sku_id, t): var for (plant, sku_id, t), var in self.d_fai.items() if var.X > POSITIVEEPS}
        self.d_fai = gp.tupledict(reserve_fai)

        self.model.remove(remove_alpha)
        self.model.remove(remove_beta)
        self.model.remove(remove_gamma)
        self.model.remove(remove_epsilon)
        self.model.remove(remove_fai) 
    ...
    def insert_pattern(self, fact_id: str, dealer_id: str, vehicle_type: str,
        ...
        for duet_sku_day, num in load_pattern.items():
            ...
            cons_dealer_total_demands_impacted = [self.cons_dealer_total_demands[dealer_id, sku_id]]
            pattern_column.addTerms(load_num, cons_dealer_total_demands_impacted)
            
            cons_dealer_demand_impacted = [self.cons_dealer_demand[dealer_id, sku_id, day]]
            pattern_column.addTerms(load_num, cons_dealer_demand_impacted)
            
            cons_plant_supply_impacted = [self.cons_plant_supply[dealer_id, sku_id, day]]
            pattern_column.addTerms(load_num, cons_plant_supply_impacted)
    ...
    def initialize_solution(self):
        ...
        for (plant, dealer, day) in triples:
            ...
            # 当前经销商需要的SKU种类和数量
            orders = {sku_id: qty for (dealer_id, sku_id), qty in self.demands.items() 
                      if dealer_id == dealer}
